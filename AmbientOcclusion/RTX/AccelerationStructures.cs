...
using AmbientOcclusion.Structs;
using System;
...
namespace AmbientOcclusion.RTX
{
    public class AccelerationStructures
    {
        ...
        public unsafe AccelerationStructureBuffers CreateTopLevelAS(ID3D12Device5 pDevice, ID3D12GraphicsCommandList4 pCmdList, AccelerationStructureBuffers[] pBottomLevelAS, ref long tlasSize)
        {
            ...
            transformation[0] = Matrix4x4.CreateScale(2, 0.01f, 2.7f);
            transformation[1] = Matrix4x4.CreateScale(0.01f, 2, 2.7f) * Matrix4x4.CreateTranslation(1.5f, 1.5f, 0);
            transformation[2] = Matrix4x4.CreateScale(0.01f, 2, 2.7f) * Matrix4x4.CreateTranslation(-1.5f, 1.5f, 0);
            transformation[3] = Matrix4x4.CreateScale(2, 2, 0.01f) * Matrix4x4.CreateTranslation(0, 1.5f, -2f);            
            transformation[4] = Matrix4x4.CreateScale(2, 0.01f, 2.7f) * Matrix4x4.CreateTranslation(0, 3, 0);
            transformation[5] = Matrix4x4.CreateScale(0.7f, 2.0f, 0.7f) * Matrix4x4.CreateFromYawPitchRoll((float)Math.PI / 10, 0, 0) * Matrix4x4.CreateTranslation(-0.5f, 0.5f, -0.5f);
            transformation[6] = Matrix4x4.CreateScale(0.7f) * Matrix4x4.CreateFromYawPitchRoll((float)-Math.PI / 10, 0, 0) * Matrix4x4.CreateTranslation(0.5f, 0.5f, 0.5f);
