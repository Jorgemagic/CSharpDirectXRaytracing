using RayTracingTutorial09.RTX;
using RayTracingTutorial09.Structs;
using System;
    ...
    public class Scene
    {
        ...
        public unsafe void CreateShaderTable()
        {
            ...
            */
            ...
            void* pCbDesc = (pData + (int)D3D12ShaderIdentifierSizeInBytes).ToPointer(); //Adding ' profIdSize' gets us to the location of the constant-buffer entry
            Unsafe.Write<ulong>(pCbDesc, (ulong)mpContantBuffer.GPUVirtualAddress);
        ...
        public unsafe void CreateConstantBuffer()
        {
            ...
            mpContantBuffer = this.acs.CreateBuffer(mpDevice, (uint)(Unsafe.SizeOf<Vector4>() * bufferData.Length), ResourceFlags.None, ResourceStates.GenericRead, AccelerationStructures.kUploadHeapProps);
            IntPtr pData;
            pData = mpContantBuffer.Map(0, null);
            Helpers.MemCpy(pData, bufferData, (uint)(Unsafe.SizeOf<Vector4>() * bufferData.Length));
            mpContantBuffer.Unmap(0, null);
        }
