using RayTracingTutorial15.RTX.Structs;
...
namespace RayTracingTutorial15.RTX
{
    public class AccelerationStructures
    {
        ...
        public void CreatePrimitive(ID3D12Device5 pDevice, out ID3D12Resource vb, out uint vertexCount, out ID3D12Resource ib, out uint indexCount)
        {
            //Primitives.Cube(1.5f, out var vertexList, out var indexList);
            Primitives.Sphere(2.0f, 32, out var vertexList, out var indexList);
            //Primitives.Pyramid(1.5f, out var vertexList, out var indexList);
            //Primitives.Torus(2.0f, 0.6f, 32, out var vertexList, out var indexList);
            ...
            vb = CreateBuffer(pDevice, (uint)(Unsafe.SizeOf<VertexPositionNormalTangentTexture>() * vertexData.Length), ResourceFlags.None, ResourceStates.GenericRead, kUploadHeapProps);
            IntPtr pData = vb.Map(0, null);
            ...
            ib = CreateBuffer(pDevice, (uint)(sizeof(ushort) * indexData.Length), ResourceFlags.None, ResourceStates.GenericRead, kUploadHeapProps);
            IntPtr pIB = ib.Map(0, null);
        ...
        {
            this.CreatePrimitive(pDevice, out var pVB, out uint vertexCount, out var pIB, out uint indexCount);
            ...
            geomDesc.Triangles.VertexBuffer.StrideInBytes = Unsafe.SizeOf<VertexPositionNormalTangentTexture>();
            geomDesc.Triangles.VertexFormat = Format.R32G32B32_Float;
            geomDesc.Triangles.VertexCount = (int)vertexCount;
            geomDesc.Triangles.IndexBuffer = pIB.GPUVirtualAddress;
            geomDesc.Triangles.IndexCount = (int)indexCount;
            geomDesc.Triangles.IndexFormat = Format.R16_UInt;
