...
namespace RayTracingTutorial17.RTX
{
    ...
    {       
        ...
        public void CreatePrimitive(PrimitiveType primitiveType, ID3D12Device5 pDevice, out ID3D12Resource vertexBuffer, out uint vertexCount, out ID3D12Resource indexBuffer, out uint indexCount)
        {
            ...
            switch (primitiveType)
            {
                ...
                case PrimitiveType.Sphere:
                    Primitives.Sphere(2.0f, 32, out vertexList, out indexList);
                    break;
                case PrimitiveType.Plane:
                    Primitives.Plane(3.0f, out vertexList, out indexList);
                    break;
                case PrimitiveType.Quad:
                    Primitives.Quad(out vertexList, out indexList);
                    break;
        ...
        public AccelerationStructureBuffers CreateBottomLevelAS(ID3D12Device5 pDevice, ID3D12GraphicsCommandList4 pCmdList)
        {
            int geometryCount = 2;
            ...
            geomDesc[0].Triangles.VertexBuffer.StartAddress = primitiveVertexBuffer.GPUVirtualAddress;
            ...
            geomDesc[0].Triangles.VertexCount = (int)primitiveVertexCount;
            geomDesc[0].Triangles.IndexBuffer = primitiveIndexBuffer.GPUVirtualAddress;
            geomDesc[0].Triangles.IndexCount = (int)primitiveIndexCount;
            geomDesc[0].Triangles.IndexFormat = Format.R16_UInt;
            geomDesc[0].Flags = RaytracingGeometryFlags.Opaque;

            // Plane
            geomDesc[1].Type = RaytracingGeometryType.Triangles;
            geomDesc[1].Triangles = new RaytracingGeometryTrianglesDescription();
            geomDesc[1].Triangles.VertexBuffer = new GpuVirtualAddressAndStride();
            geomDesc[1].Triangles.VertexBuffer.StartAddress = planeVertexBuffer.GPUVirtualAddress;
            geomDesc[1].Triangles.VertexBuffer.StrideInBytes = Unsafe.SizeOf<VertexPositionNormalTangentTexture>();
            geomDesc[1].Triangles.VertexFormat = Format.R32G32B32_Float;
            geomDesc[1].Triangles.VertexCount = (int)planeVertexCount;
            geomDesc[1].Triangles.IndexBuffer = planeIndexBuffer.GPUVirtualAddress;
            geomDesc[1].Triangles.IndexCount = (int)planeIndexCount;
            geomDesc[1].Triangles.IndexFormat = Format.R16_UInt;
            geomDesc[1].Flags = RaytracingGeometryFlags.Opaque;
        ...
        public unsafe AccelerationStructureBuffers CreateTopLevelAS(ID3D12Device5 pDevice, ID3D12GraphicsCommandList4 pCmdList, AccelerationStructureBuffers pBottomLevelAS, ref long tlasSize)
        {
            ...
            pInstanceDesc[0].Transform = Matrix4x4.Identity.ToMatrix3x4();
            pInstanceDesc[0].AccelerationStructure = pBottomLevelAS.pResult.GPUVirtualAddress;
