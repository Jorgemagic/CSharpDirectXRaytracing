...
namespace RayTracingTutorial17
{
    public class Scene
    {
        ...
        public void CreateRtPipelineState()
        {
            ...
            //  3 for hit-group (triangle hit group, plane hit-group, shadow-hit group)
            //  2 for RayGen root-signature (root-signature and the subobject association)
            //  2 for primitive hit-program root-signature (root-signature and the subobject association)
            ...
            StateSubObject[] subobjects = new StateSubObject[16];
            int index = 0;
            ...
            // Create the primitive HitProgram
            HitProgram primitiveHitProgram = new HitProgram(null, RTPipeline.kPrimitiveChs, RTPipeline.kPrimitiveHitGroup);
            subobjects[index++] = primitiveHitProgram.subObject; // 1 Primitive Hit Group
            ...
            HitProgram shadowHitProgram = new HitProgram(null, RTPipeline.kShadowChs, RTPipeline.kShadowHitGroup);
            ...
            // Create the primitive hit root-signature and association
            Structs.LocalRootSignature hitRootSignature = new Structs.LocalRootSignature(mpDevice, rtpipeline.CreatePrimitiveHitRootDesc());
            subobjects[index] = hitRootSignature.subobject; // 6 Hit Root Sig

            int primitiveHitRootIndex = index++; // 6
            ExportAssociation primitiveHitRootAssociation = new ExportAssociation(new string[] { RTPipeline.kPrimitiveChs }, subobjects[primitiveHitRootIndex]); // 5 Associate Hit Root Sig to Hit Group
            subobjects[index++] = primitiveHitRootAssociation.subobject; // 7 Associate Hit Root Sig to Hit Group

            // Create the plane hit root-signature and association
            Structs.LocalRootSignature planeHitRootSignature = new Structs.LocalRootSignature(mpDevice, rtpipeline.CreatePlaneHitRootDesc());
            subobjects[index] = planeHitRootSignature.subobject; // 8 Plane Hit Root Sig

            int planeHitRootIndex = index++; // 8
            ExportAssociation planeHitRootAssociation = new ExportAssociation(new string[] { RTPipeline.kPlaneHitGroup }, subobjects[planeHitRootIndex]);
            ...
            subobjects[index] = emptyRootSignature.subobject; // 10 Miss Root Sig
            ...
            subobjects[index++] = emptyRootAssociation.subobject; // 11 Associate Miss Root Sig to Miss Shader

            // Bind the payload size to the programs
            ShaderConfig shaderConfig = new ShaderConfig(sizeof(float) * 2, sizeof(float) * 3);
            subobjects[index] = shaderConfig.subObject; // 12 Shader Config;

            int shaderConfigIndex = index++; // 12
            string[] shaderExports = new string[] { RTPipeline.kRayGenShader, RTPipeline.kMissShader, RTPipeline.kPrimitiveChs, RTPipeline.kPlaneChs, RTPipeline.kShadowMiss, RTPipeline.kShadowChs };
            ExportAssociation configAssociation = new ExportAssociation(shaderExports, subobjects[shaderConfigIndex]);
            subobjects[index++] = configAssociation.subobject;  // 13 Associate Shader Config to Miss, CHS, RGS
        ...
        public unsafe void CreateShaderTable()
        {
            ...
            */
            ...
            // Entry 3 - Primitive hit program
            pData += (int)mShaderTableEntrySize; // +1 skips miss entries
            Unsafe.CopyBlock((void*)pData, (void*)pRtsoProps.GetShaderIdentifier(RTPipeline.kPrimitiveHitGroup), D3D12ShaderIdentifierSizeInBytes);
            ...
            // Entry 5 - Plane hit program. ProgramID only
            pData += (int)mShaderTableEntrySize;
        ...
        public bool DrawFrame(Action<int, int> draw, [CallerMemberName] string frameName = null)
        {
            ...
            raytraceDesc.MissShaderTable.SizeInBytes = mShaderTableEntrySize * 3; // 2 miss entries
